...
# com.mlops.xray integration // emirayar@
...
class ObjectDetection(GstRtspServer.RTSPMediaFactory):
    ...
    def inference(self, img):
        nn_input = img
        #Run the model
        segment = xray_recorder.begin_segment('ML_Inference', traceid=self.trace_id, parent_id=self.frame_parent_id)
        self.inference_parent_id = segment.id
        subsegment = xray_recorder.begin_subsegment('EdgeManager_PredictRequest')
        ...
        subsegment = xray_recorder.begin_subsegment('EdgeManager_Predict')
        response = edge_manager_client.Predict(request)
        xray_recorder.end_subsegment()
        subsegment = xray_recorder.begin_subsegment('process_output_tensor')
    ...
    def on_new_frame(self, sink, data):
        ...
        segment = xray_recorder.begin_segment('Video_Source_Sink')
        
        self.trace_id = segment.trace_id
        self.frame_parent_id = segment.id
        
        subsegment = xray_recorder.begin_subsegment('sink.emit')
        sample = sink.emit("pull-sample")
        xray_recorder.end_subsegment()

        subsegment = xray_recorder.begin_subsegment('process_buffer')
    ...
    def draw_overlay_cb(self, overlay, context, timestamp, duration):
        ...
        segment = xray_recorder.begin_segment('Video_Draw_Overlay', traceid=self.trace_id, parent_id=self.inference_parent_id)
        subsegment = xray_recorder.begin_subsegment('process_draw_overlay_cb')
        ...
        xray_recorder.end_segment()
        if (self.count == 30):
            segment = xray_recorder.begin_segment('Publish_AWS_IoT_Core', traceid=self.trace_id, parent_id=self.inference_parent_id)
